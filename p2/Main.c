  ...
  class ForkMonitor
    ...
    fields
      status: array [5] of int             -- For each philosopher: HUNGRY, EATING, or THINKING
		monitorLock: Mutex
		WaitingList: array [5] of Condition
    methods
      Init ()
      PickupForks (p: int)
      PutDownForks (p: int)
  ...
  behavior ForkMonitor
    ...
    method Init ()
      -- Initialize so that all philosophers are THINKING.
		...
		monitorLock.Init()			--Initialize the mutex lock
		status = new array of int { 5 of THINKING }		--Set all status to THINKING
		WaitingList = new array of Condition { 5 of new Condition}	--Create an Waiting list array with condition class
		for(i = 0; i < 5; i = i + 1)
			WaitingList[i].Init()			--Initialize
		endFor
      endMethod
    ...
    method PickupForks (p: int)	
      -- This method is called when philosopher 'p' is wants to eat.
		monitorLock.Lock()
		status[p] = HUNGRY			--Set Current philospher to Hungry status		
		self.PrintAllStatus()		
		self.test(p)				--Check to see if the neighbors are eating or not
		if(status[p] != EATING)
			WaitingList[p].Wait(&monitorLock)		--Put it into waiting list if current Thread isn't eating
		endIf
		monitorLock.Unlock()
      endMethod
	...
	method test (p: int)
		--This method will check if it can start eating if the neighbor aren't eating.
		if((status [p] == HUNGRY) && (status [(p+1) % 5] != EATING) && (status [(p-1)%5] != EATING))
			status [p] = EATING
			self.PrintAllStatus ()
			WaitingList [p].Signal (& monitorLock)		--Wake the waiting thread up.
		endIf
	endMethod
