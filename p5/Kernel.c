...
-------------------------  Hoare Condition  ---------------------------
  ...
  behavior HoareCondition
    --
    -- This class object is very similar to the Condition class
    -- The only different between these two are the implementation
    ...
    --
      ...
      method Signal(mutex: ptr to Mutex)
        ...
        -- This part will suspend the current Thread
        -- Put it into the waiting threads
        -- and resume the signaled thread
          mutex.heldBy = t
          mutex.Lock()
          oldIntStat = SetInterruptsTo (oldIntStat)
        endMethod
...
-----------------------------  ThreadManager  ---------------------------------
  ...
  behavior ThreadManager
      ...
      method Init ()
        ...
        -- Initialize the freeList and each Thread in threadTable
        -- Mark each Thread in threadTable as UNUSED iff they are on the freeList
        ...
        -- Turn the ThreadManager into a "monitor" by adding a Mutex and Condition
         ...
         -- This part is for Hoare Condition 
      ...
      method GetANewThread () returns ptr to Thread
        ...
        -- entry method must obtain the monitor lock and release at the end

          threadManagerLock.Lock()

        -- If the freeList is empty, then put put it into a wait condition
        -- Else remove the thread of the freeList, mark it as "JUST_CREATED"
        -- Then return a thread from freeList
      ...
      method FreeThread (th: ptr to Thread)
        ...
        -- 
        -- entry method must obtain the monitor lock and release at the end
        -- Mark a thread "UNUSED" and add it back to the freeList
        -- then signal anyone waiting on the Condition
...
-----------------------------  ProcessManager  ---------------------------------
  ...
  behavior ProcessManager
      ...
      method Init ()
        ...
        -- Initialize the freeList and each ProcessControlBlock in processTable and nextPid
        -- Mark each ProcessControlBlock in processTable as "FREE" iff they are on the freeList
        ...
        -- Turn the ProcessManager into a "monitor" by adding a Mutex and Condition
      ...
      method GetANewProcess () returns ptr to ProcessControlBlock
        ...
        --
          ...
          var pro: ptr to ProcessControlBlock
          processManagerLock.Lock()

        -- Similar to ThreadMeanager method
        -- Except we increment and set the processControlBlock to nextPid
        -- GetANewProcess method must change the process's status to Active
        -- after removing from the free list
      ...
      method FreeProcess (p: ptr to ProcessControlBlock)
        ...
        -- Similar to ThreadManager
        -- FreeProcess method must change the process's status to FREE
...
-----------------------------  FrameManager  ---------------------------------
  ...
  behavior FrameManager
      ...
      method GetNewFrames (aPageTable: ptr to AddrSpace, numFramesNeeded: int)
          var f, frameAddr, i: int

          frameManagerLock.Lock()

          -- Wait until we have enough free frames
          while(numFramesNeeded > numberFreeFrames)
            newFramesAvailable.Wait(&frameManagerLock)
          endWhile

          -- For each frames, determine which frame is free by using FindZeroAndSet function
          -- then find the frame address of the free frame
          -- finally, store the address of the frame which has been allocated
          ...
          -- Adjust the number of free frames and update the numberOfPages
      ...
      method ReturnAllFrames (aPageTable: ptr to AddrSpace)
         var bitNumber, i, frameAddr, numFramesReturned: int

         frameManagerLock.Lock()

         -- Go through the page table and see which frames it processed
         -- For frames it processed, find the bitNumber and clear the bit
         -- of that bitNumber
         ...
         -- broadcast all of the waiting threads and update the numberFreeFrames
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
  ...
  function Handle_Sys_Exec (filename: ptr to array of char) returns int
    -- this routine is very similair to StartUserProcess function
    -- except that this routine already have a process and a thread
    -- so we will not need to allocate a new ProcessControlBlock, Thread object, or fork.
    -- we will have to create a empty address space object

      var f_Open: ptr to OpenFile
        initPC: int
    ...
    -- The GetStringFromVirtual method is given.
    -- It calls CopyBytesFromVirtual to do the copying
      newAddrSpace.Init()
      ...
      f_Open = fileManager.Open(&strBuffer)
      if f_Open == null
        return -1
      endIf

      initPC = f_Open.LoadExecutable(&newAddrSpace)
      fileManager.Close(f_Open)
      if initPC < 0
        return -1
      endIf

    -- Free the frames first or else kernel resources will remain forever unavailable
      frameManager.ReturnAllFrames(&currentThread.myProcess.addrSpace)

    -- Copy the new address space into ProcessControlBlock
...
-----------------------------  InitFirstProcess  ---------------------------------
  function InitFirstProcess ()
    --
    -- This routine will first get a new thread object by invoking GetANewThread
    -- Then initialize the thread and invoke Fork to execute the StartUserProcess function
    -- which will do the remainder of the work in starting up a user-level process
    -- The name of the thread is used in the debugging printouts
    --
  ...
  function StartUserProcess (arg: int)
    -- Allocate a new Process Control Block and connect it with the thread
    -- Initialize the myThread field in the PCB and the myProcess field in the current Thread

     var PCB: ptr to ProcessControlBlock
       f_Open: ptr to OpenFile
       initPC: int
       logical_addrSpace: ptr to AddrSpace
       initUserStackTop: int
       initSystemStackTop: int
       oldIntStat: int
    ...
    -- create a logical address space and read the executable into it
    -- If fails, the kernel cannot start up and
    -- loadExecutable returns the entry point, called initPC.

     f_Open = fileManager.Open("TestProgram1")
     if f_Open == null
       FatalError("Can't open file")
     endIf

     logical_addrSpace = &PCB.addrSpace
     initPC = f_Open.LoadExecutable(logical_addrSpace)
     if initPC == -1
       FatalError("Invalid memory")
     endIf

     fileManager.Close(f_Open)

    -- Compute the initial value for user-level stack
    -- It should be set to the logical address just paste the end of the logical address space

     initUserStackTop = logical_addrSpace.numberOfPages * PAGE_SIZE
 
    -- full-sized system stack available for processing any syscalls or interrupts that happen later
    -- so we need to reset the system stack top pointer
    ...
    -- turn this thread into a user-level thread
     ...
     oldIntStat = SetInterruptsTo(DISABLED)
     logical_addrSpace.SetToThisPageTable()
     currentThread.isUserThread = true

    -- BecomeUserThread will jumps to the user-level main routine and never returns
     BecomeUserThread(initUserStackTop, initPC, initSystemStackTop)
