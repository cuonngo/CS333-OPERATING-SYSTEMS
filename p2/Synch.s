...
_StringConst_5:
	.word	37			! length
	.ascii	"current thread does not hold the lock"
	.align
_StringConst_4:
	.word	68			! length
	.ascii	"Current Thread asking to hold the lock while it already hold a lock."
	.align
_StringConst_3:
	.word	51			! length
...
! IF STATEMENT...
	...
	mov	"\0\0IF",r10
!   if _P_Thread_currentThread != heldBy then goto _Label_56		(int)
	set	_P_Thread_currentThread,r1
	load	[r1],r1
	load	[r14+8],r2
	load	[r2+16],r2
...
! IF STATEMENT...
	...
	mov	"\0\0IF",r10
!   if _P_Thread_currentThread == heldBy then goto _Label_68		(int)
	set	_P_Thread_currentThread,r1
	load	[r1],r1
	load	[r14+8],r2
	load	[r2+16],r2
...
! IF STATEMENT...
	...
	mov	"\0\0IF",r10
!   if intIsZero (heldBy) then goto _Label_71
	load	[r14+8],r1
	load	[r1+16],r1
	cmp	r1,r0
	be	_Label_71
...
! IF STATEMENT...
	...
	mov	"\0\0IF",r10
!   if intIsZero (temp) then goto _Label_74
	load	[r14+-32],r1
	cmp	r1,r0
	be	_Label_74
!	jmp	_Label_73
...
! ASSIGNMENT STATEMENT...
	mov	133,r13		! source line 133
	mov	"\0\0AS",r10
!   if intIsZero (temp) then goto _runtimeErrorNullPointer
	load	[r14+-32],r1
	cmp	r1,r0
	be	_runtimeErrorNullPointer
!   _temp_75 = temp + 76
...
!   _temp_76 = &_P_Thread_readyList
	set	_P_Thread_readyList,r1
	store	r1,[r14+-12]
!   Prepare Argument: offset=12  value=temp  sizeInBytes=4
	load	[r14+-32],r1
	store	r1,[r15+4]
!   Send message AddToEnd
	load	[r14+-12],r1
