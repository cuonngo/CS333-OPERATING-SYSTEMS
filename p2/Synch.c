...
-----------------------------  Mutex  ---------------------------------
  ...
  behavior Mutex
    ...
    --         on the mutex.
      ...
      method Init ()
		heldBy = null							--Set No LOCK HELD
		waitingThreads = new List[Thread]		--Create a waiting queue
        endMethod
      ...
      method Lock ()
		...
		if(currentThread == heldBy)
			FatalError("Current Thread asking to hold the lock while it already hold a lock.")
		endIf
		if(heldBy != null)		--put the currentThread into the waiting list
			waitingThreads.AddToEnd(currentThread)		--because the other thread is holding the lock
			currentThread.Sleep()						--put itself to sleep
		else
      ...
      method Unlock ()
		...
		oldIntStat = SetInterruptsTo(DISABLED)
		if(currentThread != heldBy)
			FatalError("current thread does not hold the lock")
		endIf
		if(heldBy != null)			--Unlock the thread by checking if it is held by any thread.
			temp = waitingThreads.Remove()	--wake up the thread in front of queue
			if(temp !=null)		--make sure the waiting threads are not empty
				temp.status = READY
				readyList.AddToEnd(temp)
				heldBy = temp
			else
				heldBy=null
			endIf
